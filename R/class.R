...
#' @examples
#' class1()
class1 <- function(clusters,KNN,symmtery=FALSE){
  ...
  for (i in 1:nrow(KNN)){
    ...
    for (j in 1:ncol(KNN)) {
      idx2 = KNN[i,j]
      if(symmtery==FALSE) {class1[idx1, idx2] = class1[idx1, idx2] + 1}
      if(symmtery==TRUE) {class1[idx1, idx2] = (class1[idx1, idx2] + 1) + t(class1[idx1, idx2] + 1)}
